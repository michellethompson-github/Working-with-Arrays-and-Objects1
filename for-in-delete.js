...
function showValues(obj) {
  for(var key in obj) //Code Here
  {
    // console.log(obj[key]); //Here
    text += obj[key]; //These are the property

    //text = text + values[key] //text = text + values[key]
  }
...
//Code Here
//In for in loops, we have three things in the loop
//obj: which is the object --- Can be accessed using obj
//key: which is the key/property --- can be accessed using key
//value: which is the value associated to a specific key --- can be accessed using obj[key]
function secrets(obj){
 var secretsOne = '';
 for(let key in obj){
   if(key.startsWith('sh', 0)){
    secretsOne += obj[key]
   }
 }
 return secretsOne
...
// delete deleteTheBigNumbers[fourth]
...
for(let key in deleteTheBigNumbers){
 if(deleteTheBigNumbers[key] > 100){
  delete deleteTheBigNumbers[key]; //delete objectname[keyname]
    }
  }
...
function hiddenTreasure(obj){
  for(let key in obj){
    if(!(obj[key].includes('treasure'))){ //The ! operator returns the opposite
      delete obj[key]
    }
  }
  return obj;
