...
let evenNumbers = mixedNumbers.filter ((element, index, wholeArray) => { // = mixedNumbers.filter(/* Provide Your Callback Here */)
  if(element % 2 === 0){ //10 / 2 : remainder : 0
    return element;
  }
});  
...
let totalPopulation = populations.reduce(reducer)//  = populations.reduce(/* Provide Your Callback Here */)
...
// Do not edit the code below.
const monstersInYourPocket = [{"monster":"Bulbabunny","CP":156},
{"monster":"Bulbabunny",
"CP":135},
...
//Code Here
//let myStrongest = monstersInYourPocket.filter (monstersInYourPocket => // = monstersInYourPocket.filter(/* Provide Your Callback Here */)
...
let orderTotals = orders.map( x => x.price * (1 + x.tax));// = prices.map(/* Provide Your Callback Here );
...
var bobsTotal = 0;
let bobs_array = purchases.filter ((element, index, wholeArray) =>  {
  if(element.owner === "Bob"){
  bobsTotal = bobsTotal + element.price
  return element;

  }
})
// console.log(bobs_array);
