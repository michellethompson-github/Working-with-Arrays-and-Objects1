...
var employees = [ //Array of Objects
  {
    "firstName": "Von",
    "lastName": "Budibent",
...
function employeeUpdater() {
  employees.forEach(function(item, index){
    if(item.firstName === 'Theo'){
      employees.splice(index, 1); //spliace(indexNumber, how much)
    }
  ...
  for(let i=0; i < workplaceAccidents.length; i++){
    for(let j=0; j < workplaceAccidents.length; j++){
      ...
      if(workplaceAccidents[i] === workplaceAccidents[j]){
        console.log('Duplicate Number is : ', workplaceAccidents[i])
...
//Code Here
var grumpyActivity = cat.catFriends[0].activities[1]; //Assign eat food
...
function recordCleaner(){
  myCar.accidents.forEach(function(item, index){
...
//Code Here
function looper(){
  // numsArr.forEach(function(item, index){
  //   item.forEach(function(inneritem, location){
  //     console.log(inneritem);
  //   })
  // })
  for(var i=0; i < numsArr.length; i++){
    for(var j=0;j< numsArr[i].length; j++){
      if(numsArr[i][j] % 2 === 0) {//it is an even
      numsArr[i][j] = 'even'
    }
    else {
      numsArr[i][j] = 'odd'
